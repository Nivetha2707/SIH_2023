...
"""
cursor_leak.execute(create_table_query)
...
def generate_and_insert_combined_data():
    ...
    while True:
        ...
        if data_leak is not None and data_no_leak is not None:
            ...
            nrw_diff = data_no_leak[2] - data_leak[2]
            flowrate_diff = data_no_leak[1] - data_leak[1]

            # Insert data into non_revenue_water
            insert_query = """
                INSERT INTO non_revenue_water (timestamp, nrw, total_nrw, flowrate_diff)
                VALUES (%s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE nrw=%s, total_nrw=%s, flowrate_diff=%s
            """
            insert_data = (data_leak[0], nrw_diff, total_nrw + nrw_diff, flowrate_diff, nrw_diff, total_nrw + nrw_diff, flowrate_diff)
            cursor_leak.execute(insert_query, insert_data)
