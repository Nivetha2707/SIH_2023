            ...
            if result:
                ...
                if nrw_value > 0.1 and time_difference < timedelta(minutes=1):
                    ...
                    if sensor2_location:
                        ...
                        leak_result = execute_second_code()
                        # Send SMS using Twilio
                        client = Client(twilio_account_sid, twilio_auth_token)
                        message = client.messages.create(
                            body=f"High NRW alert: NRW value is {nrw_value}, Flowrate Diff: {flowrate_diff},Sensor's Latitude: {latitude}, Sensor's Longitude: {longitude}, at {timestamp}, Result: {leak_result}",
